/* =========================================================
   ...
   REGULAR MARKETPLACE COUNTRIES DATABASE
   ---------------------------------------------------------
   Version: 24.6 SAFE CENTRAL MARKETPLACE COUNTRY SYSTEM
   Creator: Baba Thecno Guru

   PURPOSE:
   - Regular Marketplace country database
   - Global Marketplace compatibility
   - Jobs/Careers compatibility
   - One central country source
   - Bangladesh and Pakistan excluded
   ========================================================= */
...
(function () {
    ...
    window.ALON_MARKETPLACE_COUNTRIES = MARKETPLACE_COUNTRIES;
    ...
    window.ALON_COUNTRY_LABEL = function (country) {
        ...
        return [
            ...
            "(" + (country.iso2 || country.code2 || "") +
            " • " +
            (country.iso3 || country.code3 || "") + ")",
            country.callingCode || country.phoneCode || ""
        ].join(" ").trim();
    };

    window.ALON_FILL_COUNTRY_SELECT = function (selector, selectedValue) {
        let select = selector;
        ...
        const oldValue = selectedValue !== undefined
        ...
        if (oldValue) {
            const found = Array.from(select.options).find(function (option) {
                return (
                    ...
                    option.textContent.toLowerCase().includes(oldValue.toLowerCase())
                );
            });
    ...
    document.addEventListener("DOMContentLoaded", fillMarketplaceSelects);
    window.addEventListener("load", fillMarketplaceSelects);
